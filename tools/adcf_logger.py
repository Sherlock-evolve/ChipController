...
# -*- coding: utf-8 -*-
"""
ChipController 无 Qt 上位机简易采集脚本。

每隔 N 秒（默认 1s）向 ChipController 的 Type-C 调试串口发送 `adcf 1`，
解析返回的一阶低通滤波 ADC 读数（I / V / R），
用当前项目的 TCR/R-T 表把同一条滤波电阻换算为芯片温度，
连同时间戳追加写入与 Qt 上位机兼容的 adcf_log_*.csv。
CSV 列为: timestamp, I_nA, V_uV, R_uOhm, chip_temp_C, note。

控温曲线使用 tools/plot_temperature_resistance.py 绘制。需要包含单调时钟、
采样耗时和 ADC 状态的板级长期温漂测试时，请使用
tools/temp_drift_test.py + tools/plot_temp_drift.py。
...
def main():
    p = argparse.ArgumentParser(
        description="ChipController 无 Qt 简易采集 (adcf 1 -> Qt 兼容 CSV)")
