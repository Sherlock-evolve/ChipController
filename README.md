...
- **双闭环控制**：恒流模式（0–20 mA）与恒温模式（−200 … +100 °C，PI 调节，带抗积分饱和）。
- **多重安全保护**：过流 / 过压 / 过功率 / 过温四级硬件级跳闸，驱动电压斜率限制。
...
`Core/Src/chip_temperature.c` 内置分度表（21–37 Ω ↔ −175 … +23 °C），按测得电阻做分段线性插值，
仅在电流有效时输出温度。
...
控制环（`thermal_control.c`）在每个服务周期（500 ms）检查，超限即跳闸并归零：
...
> 使用 150 Ω 量级标准电阻标定时，13 mA ≈ 1.95 V 已接近 2 V 过压保护，请勿继续提高电流。
