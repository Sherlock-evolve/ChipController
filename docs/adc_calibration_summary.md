...
13 mA 的理论负载电压已超过 1.9 V 软限压点，因而不纳入标定。
...
误差 RMS 约为 1.04 mΩ，最大约为 1.64 mΩ。2026-08-07 的独立递增扫描发现各档
仍有约 `-3.186 mΩ` 的一致残差，因此保留有效零点，仅将电流增益下调 21.25 ppm：

```text
CHIP_MEASURE_CURRENT_ZERO_A = -0.800737 µA
CHIP_MEASURE_CURRENT_GAIN   = 1.000719600
```

新系数对该次扫描各档最后 60 s 数据的回算误差 RMS 为 0.736 mΩ，最大绝对误差
为 1.270 mΩ。
...
`adcs`、控制反馈和 `adcf` 使用相同的板级校准路径。控制反馈使用独立的
`alpha = 0.25` 一阶低通，以缩短切档后的收敛时间；`adcf`、Qt 显示和 CSV 日志
仍使用 `alpha = 0.05` 抑制显示波动。滤波均位于板级校准之后，因此不会改变稳态
直流比例，也无需重新计算现有零点和增益系数。零点测量优先使用 `adcs` 观察原始
偏置和噪声，带载增益标定及 Qt 最终验收使用 `adcf` 稳态数据。过流、软限压、
硬过压和过功率判断直接使用未滤波采样值，避免滤波延迟保护响应。
...
13 mA 已超过 `THERMAL_CONTROL_SOFT_LOAD_VOLTAGE_V = 1.900 V`，控制器会禁止驱动
继续上升；14 mA 的理论负载电压则已经超过 2.000 V 锁存硬保护阈值。

### 当前增益来源

`CHIP_MEASURE_CURRENT_GAIN = 1.000719600f`

- 初始拟合来源：`adcf_log_20260806_162559.csv`
- 最终微调来源：[`adcf_log_20260807_101653.csv`](../adcf_log_20260807_101653.csv)
- 参考负载：150.002 Ω 四线精密电阻
- 微调范围：首次递增扫描 0.3、0.5、0.7、1、2、3、5、7、10、12 mA
- 方法：每档最后 60 s 的 I/V 稳态数据，各档电阻误差等权，仅调整电流增益
- `CURRENT_ZERO = -0.800737 µA` 保持不变
...
对 [`adcf_log_20260807_101653.csv`](../adcf_log_20260807_101653.csv) 首次递增扫描
每档最后 60 s 数据应用最终电流系数后，按固件相同公式回算如下：

| 目标电流 | 微调前误差 | 最终系数回算误差 |
| ---: | ---: | ---: |
| 0.3 mA | -3.527 mΩ | -0.338 mΩ |
| 0.5 mA | -3.008 mΩ | +0.180 mΩ |
| 0.7 mA | -3.047 mΩ | +0.142 mΩ |
| 1 mA | -3.783 mΩ | -0.595 mΩ |
| 2 mA | -2.110 mΩ | +1.078 mΩ |
| 3 mA | -2.159 mΩ | +1.029 mΩ |
| 5 mA | -2.764 mΩ | +0.424 mΩ |
| 7 mA | -2.915 mΩ | +0.273 mΩ |
| 10 mA | -4.084 mΩ | -0.895 mΩ |
| 12 mA | -4.459 mΩ | -1.270 mΩ |

10 档平均误差由 -3.186 mΩ 修正为约 +0.003 mΩ；回算误差 RMS 为 0.736 mΩ，
最大绝对误差为 1.270 mΩ。该结果是写入前回算，仍需烧录后按相同流程实机复核。
...
THERMAL_CONTROL_SOFT_LOAD_VOLTAGE_V  = 1.900f; // 1.9 V non-latching clamp
THERMAL_CONTROL_MAX_LOAD_VOLTAGE_V   = 2.000f; // 2 V
THERMAL_CONTROL_MAX_POWER_W          = 0.100f; // 100 mW
BOARD_OUTPUT_MAX_DRIVE_V             = 1.000f; // 1 V DAC drive command
```

其中 1.9 V 软限压使用未滤波负载电压判断：达到阈值后只禁止驱动继续上升，允许
控制环向下修正，并报告 `LIMIT_CLAMPED`，不会锁存故障。2.0 V 是独立的锁存硬保护，
触发后立即归零输出。使用 150.002 ohm 负载时：

```text
13 mA -> 1.950 V，超过软限压点，无法作为正常稳态标定档
14 mA -> 2.100 V，超过硬过压阈值
```
